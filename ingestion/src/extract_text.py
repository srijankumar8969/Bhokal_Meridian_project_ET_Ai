...
def extract_pdf_text(filepath):
    """Extract text from a digital (non-scanned) PDF, page by page."""
    text_by_page = []
    with pdfplumber.open(filepath) as pdf:
        for page_num, page in enumerate(pdf.pages, start=1):
            page_text = page.extract_text() or ""
            text_by_page.append({"page": page_num, "text": page_text.strip()})
    return text_by_page
    ...
    for filename in os.listdir(DATA_DIR):
        ...
        try:
            if ext == "pdf":
                ...
                # Skip PNG and anything else — handled separately (OCR day)
